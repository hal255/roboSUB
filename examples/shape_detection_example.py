...
while True:
    ...
    for cnt in contours:
        approx = cv2.approxPolyDP(cnt,approx_variable *cv2.arcLength(cnt,True),True)
    ...
    # if button 1 is pressed, approx_variable + 0.01
    if k == 49:
        approx_variable += 0.01
        print "approx_variable=", approx_variable

    # if button q is pressed, approx_variable - 0.01
    if k == 113:
        approx_variable -= 0.01
        if approx_variable < 0.01:
            approx_variable = 0.01
        print "approx_variable=", approx_variable
